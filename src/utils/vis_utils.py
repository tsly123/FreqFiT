...
def get_df(files, model_type, root, is_best=True, is_last=True, max_epoch=300, no_test=False):
    ...
    for job_path in tqdm(files, desc=model_type):
        ...
        if len(np.where(eval_results[metric_b] == np.max(eval_results[metric_b]))[0]) > 1:
            top1_id = np.where(eval_results[metric_b] == np.max(eval_results[metric_b]))[0].tolist()
            test_top1 = []
            for t in range(len(eval_results['test_top1'])):
                if t in top1_id:
                    test_top1.append(eval_results['test_top1'][t])
            best_epoch = top1_id[np.argmax(test_top1)]
...
def average_df(df, metric_names=["l-val_top1", "l-val_base_top1"], take_average=True):
    ...
    hp_names = [
        c for c in df.columns if c not in ["data", "feature", "file", "best_epoch"] + metric_names
    ]
    ...
    df = df.sort_values(["data", "feature"])
    return df
