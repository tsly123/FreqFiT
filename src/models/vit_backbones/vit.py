...
class Encoder(nn.Module):
    def __init__(self, config, vis, freqfit_config=None):
        ...
        if self.freqfit_config == "freqfit":
            ...
            self.ssf_scale, self.ssf_shift = init_ssf_scale_shift(config.transformer["num_layers"]+1, config.hidden_size)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(config, img_size, vis, freqfit_config)
