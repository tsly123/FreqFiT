...
class ADPT_Encoder(nn.Module):
    def __init__(self, config, vis, adapter_cfg, freqfit_config):
    ...
    def forward(self, hidden_states):
        ...
        for i, layer_block in enumerate(self.layer):
            if self.freqfit_config == "freqfit":
                hidden_states = self._filter_ops(i, hidden_states)  # freqfit
            elif self.freqfit_config == "ssf":
                hidden_states = ssf_ada(hidden_states, self.ssf_scale[i], self.ssf_shift[i])    # ssf
...
class ADPT_Transformer(nn.Module):
    def __init__(self, config, img_size, vis, adapter_cfg, freqfit_config):
        super(ADPT_Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size)
        self.encoder = ADPT_Encoder(config, vis, adapter_cfg, freqfit_config)
...
class ADPT_VisionTransformer(nn.Module):
    def __init__(
        self, model_type,
        img_size=224, num_classes=21843, vis=False, adapter_cfg=None, freqfit_config=None
    ):
        ...
        self.transformer = ADPT_Transformer(config, img_size, vis, adapter_cfg, freqfit_config)
